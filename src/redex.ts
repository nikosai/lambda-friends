...
export abstract class Redex {
  ...
  public addLeft(s: string): void {
    this.left = s + this.left;
  }
  public addRight(s: string): void {
    this.right += s;
  }
  public addTexLeft(s: string): void {
    this.texLeft = s + this.texLeft;
  }
  public addTexRight(s: string): void {
    this.texRight += s;
  }
  public static makeNext(
    es: Redex[],
    prefix: string,
    suffix: string,
    prefixTex: string,
    suffixTex: string,
    func: (prev: Expression) => Expression
  ): Redex[] {
    const ret: Redex[] = [].concat(es);
    for (const e of ret) {
      e.next = func(e.next);
      e.addLeft(prefix);
      e.addRight(suffix);
...
export class BetaRedex extends Redex {
  ...
  public toString(): string {
    ...
    const str = boundvars.join('') + '.' + expr.toString(true);
    return (
      this.left +
      '(\\[' +
      this.la.boundvar.toString(false) +
      ']' +
      str +
      ')[' +
      this.arg.toString(false) +
      ']' +
      this.right
    );
  }
  public toTexString(): string {
    ...
    const str = boundvars.join('') + '.' + expr.toTexString(true);
    return (
      this.texLeft +
      '(\\strut \\lambda{\\underline{' +
      this.la.boundvar.toTexString(false) +
      '}}' +
      str +
      ')\\underline{\\strut ' +
      this.arg.toTexString(false) +
      '}' +
      this.texRight
    );
  }
  public toHTMLString(): string {
    ...
    const str = boundvars.join('') + '.' + expr.toString(true);
    return (
      Util.htmlEscape(this.left) +
      '(\\<span class="lf-beta lf-boundvar">' +
      Util.htmlEscape(this.la.boundvar.toString(false)) +
      '</span>' +
      Util.htmlEscape(str) +
      ')<span class="lf-beta lf-arg">' +
      Util.htmlEscape(this.arg.toString(false)) +
      '</span>' +
      Util.htmlEscape(this.right)
    );
  }
...
export class EtaRedex extends Redex {
  ...
  public toTexString(): string {
    return (
      this.texLeft +
      '\\underline{\\strut ' +
      this.content.toTexString(this.left + this.right === '') +
      '}' +
      this.texRight
    );
  }
  public toHTMLString(): string {
    return (
      Util.htmlEscape(this.left) +
      '<span class="lf-eta">' +
      Util.htmlEscape(this.content.toString(this.left + this.right === '')) +
      '</span>' +
      Util.htmlEscape(this.right)
    );
  }
...
export class MacroRedex extends Redex {
  ...
  public toTexString(): string {
    return (
      this.texLeft +
      '\\underline{\\strut ' +
      this.content.toTexString(false) +
      '}' +
      this.texRight
    );
  }
  public toHTMLString(): string {
    return (
      Util.htmlEscape(this.left) +
      '<span class="lf-macro">&lt;' +
      Util.htmlEscape(this.content.name) +
      '&gt;</span>' +
      Util.htmlEscape(this.right)
    );
  }
...
export class TypedRedex extends Redex {
  ...
  public toString(): string {
    return (
      this.left +
      '[' +
      this.content.toString(this.left + this.right === '') +
      ']' +
      this.right
    );
  }
